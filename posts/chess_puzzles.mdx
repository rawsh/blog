...
It's been shown that combining fine-tuning and prompt optimization can be powerful [^2]. To see if the performance of chat models could be brought up to a level competetive with completion models, I ran several experiments. I tried automatically optimizing the LLM chess puzzle solving prompt with tuned few shot examples, and in light of OpenAI's 2 million tokens of free gpt-4o-mini finetuning per day, I also tried finetuning gpt-4o-mini on the optimized LLM chain of thought outputs.

## Contents

## Dataset

To create a dataset to evaluate the models on, I used puzzles from the lichess database, which provides a dataset of 4 million chess puzzles with metadata. For each puzzle, I called the API to get the full list of moves in PGN format, with one example for each move of the puzzle.

```
PGN: 1. e3 e5 2. Ne2 d5 3. Ng3 Nf6 4. Nh5 Bd6 5. d3 Bg4 6.
Answer: ?
```
solution (hint: knight move) [^3]

The dataset is available on [huggingface](https://huggingface.co/datasets/rawsh/pgnpuzzles).
...
### DSPy Program
Using DSPy, few-shot examples are compiled to create an optimized prompt. This approach alone improves gpt-4o-mini's accuracy to 25.99%, a 50% increase over the zero-shot baseline. When combined with a self-correction step for illegal moves and 2 retries, the accuracy further improves to 31.72%.
...
By optimizing over few shot examples with DSPy, gpt-4o-mini performance is boosted to 31.72%. Using the optimized few shot chain of thought prompt with gpt-4o brings some impressive gains, bringing the accuracy up to 63.88%.
...
DSPy optimized chain of thought prompt with five few shot examples:
```json
{
  "generate_move": {
    "lm": null,
...
By running the best pipeline we have available (gpt-4o with the compiled prompt), we can construct fine tuning examples with the chain of thought processes from the strong model. 
...
![llm model accuracy on chess puzzle dataset](/model_accuracy.png)
